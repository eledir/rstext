...
public class PolaritySuperSense2 {
	...
	private Map<String, List<String>> mapAll;


	
	public PolaritySuperSense2(SentenceModel sm,
	...
	public void computePolarity() throws JWNLException {
		...
		for (String verb : setVerb) {
			...
			for (String itemNC : verbNounContext) {
				...
				if (index >= 0) {
					...
					if (this.sentenceModel.getIsValidWord().get(index)) {
						...
						String lemma = this.sentenceModel.getLemma().get(index).toLowerCase();
						...
						if(mapAll.containsKey(lemma)){
							...
							List<String> list=mapAll.get(lemma);
							
							if(!list.get(1).equals("#")){
							
								value= new Integer(list.get(1));
								
							}
							else{
							
								value= new Integer(list.get(0));
			...
			if (this.mapNegation.containsKey(verb)&&polarity!=0) {
